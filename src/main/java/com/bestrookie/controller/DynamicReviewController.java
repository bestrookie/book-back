...
public class DynamicReviewController {
    ...
    public MyResult getDynamicReview(HttpServletRequest request, HttpServletResponse response){
        MyResult result;
        if (Integer.parseInt(request.getParameter("pageNumber")) < 0 || Integer.parseInt(request.getParameter("pageSize")) < 0 || Integer.parseInt(request.getParameter("dId")) < 0){
            result = MyResult.failed("参数错误",null,411);
        }else {
    ...
    public MyResult deleteDynamicReview(HttpServletResponse response,HttpServletRequest request){
        ...
        if (dynamicId > 0 && reviewId > 0 && userId > 0 ){
